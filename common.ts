...
import { readableStreamToString, AsanaError, parseIfJson } from "./error";
    ...
    const authenticate = async () => {
        if (configuration.oauthClientId && configuration.oauthClientSecret && configuration.accessToken === undefined) {
            const token = await wrapAxiosRequest(async () => {
                const url = configuration.oauthTokenUrl ?? "https://app.asana.com/-/oauth_authorize"
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new AsanaError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new AsanaError(e, e.response?.data)
                }
                if (innerError instanceof AsanaError) {
                    // Got "AsanaError" from the above try block
                    throw innerError;
                }
